...
(defun cnesl-func (name exp precedence)
  (let* ((realname (if (pscode-p name) 
		       (code-name (pscode-serial name))
		     name))
...
(defun cnesl-print-nested-sequence (self)
  ...
  (let ((data (nested-sequence-data self))
	...
	(if (= (length data) 0)
	    ...
	    (format nil "[]")
	  (format nil (cond ((= *max-print-length* 0) "[...]")
...
(defun cnesl-print-nesl-struct (self &optional (paren-p nil))
  (let* ((data (nesl-struct-data self))
	 ...
	 (type (if (pscode-p type) (code-name (pscode-serial type)) type)))
    (if (eql type 'pair)
...
(defun eql-pscode (val1 pscode-name)
  (and (pscode-p val1)
       (eql (code-name (pscode-serial val1)) pscode-name)))

(defun cnesl-exp (a &optional (precedence 0))
  (cond ((pscode-p a)
	 (new-line (string-downcase (code-name (pscode-serial a)))))
	((atom a) 
...
(defun cnesl-function (function)
  (let ((arg-lines (cnesl-exp (cons (func-name function)
    ...
    (append (append-lines (new-line "function ") arg-lines 
			  type-lines (new-line " ="))
	    (inc-lines (append-lines body-lines (new-line ";"))
		       2))))
...
(defun pprint-nesl-string-rec (lines indent max-lines)
  (if (null lines) ""
    (if (= 0 max-lines) (format nil "~%  .....")
      (concatenate 
       'string 
       (let* ((line (car lines))
	 ...
	 (format nil "~%~a~a" 
		 ...
		 (cdr line)))
       (pprint-nesl-string-rec (cdr lines) indent (- max-lines 1))))))

(defun pprint-oneline-string (a)
  (let ((str (pprint-nesl-string-rec a 0 -1)))
    (subseq str 1 (length str))))

(defun pprint-nesl-string (exp &optional (indent 0))
  (pprint-nesl-string-rec (cnesl-exp exp) indent -1))
...
(defun pprint-nesl-result (val type vars)
  (pprint-nesl-string-rec 
   (append-lines (cnesl-exp vars 1)
		 (new-line " = ")
		 (new-line (cnesl-print-data (nest-constant val) t))
		 (new-line " : ")
