...
(defparameter *already-dumped* nil)
...
(defun eval-set (command definitions)
  (let ((com (cdr (assoc (car command) *set-commands*))))
    ...
    (cond ((eql (car command) 'trace)
	  ...
	  ((eql (car command) 'debug)
	   (setq *debug* (onoff? (second command) command)))
	  ;((eql (car command) 'editor)
  ...
  (cond ((func-p form)
	...
	((listp form)
	 (cond ((eql (car form) 'set) (eval-set (cdr form) defs))
	       ...
	       ;((eql (car form) 'progn) (eval form))
	       ;((eql (car form) 'shell) (eval-shell (cdr form)))
	       ((eql (car form) 'pprint) (pprint-nesl  (second form)))
	       ((eql (car form) 'assign) (eval-assign (cdr form) defs))
	       ;((eql (car form) 'file-assign) 
		...
		(add-prim-type (second form) defs))
	       ;((eql (car form) 'edit) (eval-edit form defs))
		...
		(pprint-nesl (conv-exp (strip-exp (second form) defs) t nil)))
	       ;((and (eql (car form) 'dump) (eql (second form) 'xxxxx)
	       ; (not *already-dumped*))
	       ;(save-lisp-image
	       ;"/afs/cs.cmu.edu/project/scandal/nesl/server/nesl" nil)
	       ;(setq *already-dumped* t))
...
(defun init-nesl (defs)
  ...
  (let ((*package* (find-package 'nesl-lisp)))
    ...
    ;;(when (probe-file "~/.nesl") (load-nesl "~/.nesl"))
...
(defun user::dump-nesl-server (filename)
  ...
  (save-lisp-image filename #'run-nesl-filter))
