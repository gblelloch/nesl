...
(defun cskip-whitespace ()
  (do ((commentp nil)
      ...
      (cond ((eql c (if commentp *end-comment-char* *begin-comment-char*))
	     (identity (setq commentp (not commentp))))
	    ((cwhitespacep c)
...
(defun make-token (l rp fixnum-p flonum-p)
  ...
  (prog1
      (cond (fixnum-p (readlist l))
	    ...
	    ;; The or is a hack to deal with the symbol nil to
	    ;; make sure it is not treated as the empty list
	    (t (or (intern (coerce l 'string)) '|nil|)))
    (reklaim l)
    ))
