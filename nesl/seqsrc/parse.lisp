	   ...
	   (list 'load-nesl 
		 (match-args (parse 3) 
			     (list-pair 'exp '(|:=| verbose t)
					'(|:=| print f)
					'(|:=| save_filename t))))))
	((check-token 'obj_load)
	 (if (end-token) (list 'nload-nesl)
	   (list 'nload-nesl 
		 (match-args (parse 3) 
			     (list-pair 'exp '(|:=| verbose t)
					'(|:=| print f)
...
(defconstruct datatype
  (let* ((typename (get-token))
    ...
    (list 'defrec typename (car type) (cdr type))))

(defun parse-empty-sequence ()
  (let ((*currently-parsing-type* t))
    (declare (special *currently-parsing-type*))
    (list 'make-empty-vector (parse 29))))
...
;	  `(let (foo (,(get-pscode 'start_timer) 0))
;	     (pair ,(parse 3) (,(get-pscode 'stop_timer) 0)))
	  (let* ((sym (gentemp "time-")))
	    `(let (,sym (,(get-pscode 'start-timer) 0))
	       (pair ,(parse 3) (,(get-pscode 'stop-timer) ,sym))))
	(let () 
...
(add-infix-fun '&= 2 2
  (cgolerr "&= is not supported in the sequential version."))
...
(add-infix-fun '|\|=| 2 2
   (cgolerr "The |= is not supported in the sequential version.  Use = instead."))
